...
predator_initial <- initial_densities %>%
    filter(Prey == prey & Super == super & Apex == apex) %>%
    select(sample_id, Predator) %>%
    distinct() %>%
    rename(predator_initial = Predator)

predator_initial

summary(predator_initial)

data <- filter(data, sample_id %in% predator_initial$sample_id)
...
time_series_bold <- data %>%
    filter(sample_id %in% predator_initial$sample_id, rep_id == r) %>%
    select(sample_id, step, Prey, Predator) %>%
    left_join(predator_initial, by = "sample_id")

time_series <- data %>%
    filter(sample_id %in% predator_initial$sample_id) %>%
    select(sample_id, step, Prey, Predator) %>%
    left_join(predator_initial, by = "sample_id")
...
ggplot(time_series, aes(x = step, y = Prey)) +
    ...
    facet_wrap(~predator_initial, scales = "free_y") +
    theme_bw()
...
ggplot(bifurcation, aes(x = predator_initial, y = Prey)) +
