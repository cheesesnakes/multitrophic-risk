...
super_initial <- initial_densities %>%
    filter(Prey == prey & Predator == predator & Apex == apex) %>%
    select(sample_id, Super) %>%
    distinct() %>%
    rename(super_initial = Super)

super_initial

summary(super_initial)

data <- filter(data, sample_id %in% super_initial$sample_id)
...
time_series_bold <- data %>%
    filter(sample_id %in% super_initial$sample_id, rep_id == r) %>%
    select(sample_id, step, Prey, Predator) %>%
    left_join(super_initial, by = "sample_id")

time_series <- data %>%
    filter(sample_id %in% super_initial$sample_id) %>%
    select(sample_id, step, Prey, Predator) %>%
    left_join(super_initial, by = "sample_id")
...
ggplot(time_series, aes(x = step, y = Prey)) +
    ...
    facet_wrap(~super_initial, scales = "free_y") +
    theme_bw()
...
ggplot(bifurcation, aes(x = super_initial, y = Prey)) +
    geom_jitter(alpha = 0.25) +
    geom_hline(yintercept = 0, linetype = "dashed") +
    geom_vline(xintercept = 0, linetype = "dashed") +
    theme_bw()

ggplot(bifurcation, aes(x = super_initial, y = Predator)) +
    geom_jitter(alpha = 0.25) +
    geom_hline(yintercept = 0, linetype = "dashed") +
    geom_vline(xintercept = 0, linetype = "dashed") +
    theme_bw()
